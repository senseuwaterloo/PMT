...
public class Injector {
	...
	void inject_mutant_to_ast_rewriter(ASTNode node)
		...
		SimpleName replacement = build_replacement_node(node, myParent.rewriter.getAST());
...
//		TODO
		...
		MethodDeclaration parent_method = null;
		if (pmt.utilities.Utilities.findParentMethodDeclaration(node) != null) {
			parent_method = (MethodDeclaration) pmt.utilities.Utilities.findParentMethodDeclaration(node);
		}
		if (parent_method != null) {
			MethodInvocation replacement = (MethodInvocation) build_logging_node(node, myParent.rewriter.getAST(),
					myReplacement);
			Block block = parent_method.getBody();
	...
	@SuppressWarnings("unchecked")
	SimpleName build_replacement_node(ASTNode node, AST ast) {
		SimpleName string_buffer = ast.newSimpleName("StringBuffer");
